module.exports = function(options) {
	var mysql = require("mysql");
	  ...
	  var resList = {results: []};
	  con.query('select year(ENTRY_DATE) as anno, month(ENTRY_DATE) as mese, round(sum(HOURS)/8,2) as giornateMese from TIMESHEET_ENTRY t join PROJECT_ASSIGNMENT a on t.ASSIGNMENT_ID = a.ASSIGNMENT_ID join PROJECT p on a.PROJECT_ID = p.PROJECT_ID join USERS u on u.USER_ID = a.USER_ID join CUSTOMER c on p.CUSTOMER_ID = c.CUSTOMER_ID group by anno, mese having anno = year(now())', function(err,rows){
			    if (err) throw err;
